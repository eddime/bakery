...
cmake --build . --target bakery-launcher-shared -j4

if [ ! -f "bakery-launcher-shared" ]; then
    echo "❌ ARM64 launcher build failed!"
    exit 1
fi

mv bakery-launcher-shared "$BUILD_DIR/bakery-arm64"
...
mv bakery-launcher-shared "$BUILD_DIR/bakery-x86_64"
